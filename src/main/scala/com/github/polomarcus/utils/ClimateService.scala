...
object ClimateService {
  ...
  def getMinMax(list: List[CO2Record]): (Double, Double) = {
    if (list.isEmpty) {
      ...
      val validPpmValues = list.filter(_.ppm >= 0).map(_.ppm)

      if (validPpmValues.isEmpty) {
        (0.0, 0.0)
      } else {
        val minPpm = validPpmValues.min
        val maxPpm = validPpmValues.max
        (minPpm, maxPpm)
      }
    }
  }
  ...
  def calculateDifference(list: List[CO2Record]): Double = {
    val (minValue, maxValue) = getMinMax(list)
    maxValue - minValue
  }
  /**
  ...
  def showCO2Data(list: List[Option[CO2Record]]): Unit = {
    ...
    val CO2_list = filteredList.map(_.show())
    println(CO2_list)
  ...
  def estimateCO2LevelsFor2050(recordsWithOptions: List[Option[CO2Record]]): Double = {
    // Filter out None values and extract valid CO2Record instances
    val records: List[CO2Record] = recordsWithOptions.flatten

    // Throw an exception if no valid CO2 records are provided
    if (records.isEmpty) {
      throw new IllegalArgumentException("No valid CO2 records provided for estimation")
    }
    ...
    // Predict the CO2 level for the year 2050
